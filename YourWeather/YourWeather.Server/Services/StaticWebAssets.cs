...
{
    public class StaticWebAssets : Rcl.Services.StaticWebAssets
    {
        ...
        public override async Task<string> ReadContentAsync(string relativePath, bool isRcl = true)
        {
            ...
            if (isRcl)
            {
                path = $"{AppContext.BaseDirectory}/wwwroot/_content/{RclAssemblyName}/{relativePath}";
            }
            else
            {
#endif
            path = $"{AppContext.BaseDirectory}/wwwroot/{relativePath}";
#if !DEBUG
            }
#endif
